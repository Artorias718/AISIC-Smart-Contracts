...
contract AISIC is ERC20("Test","TT"),Ownable{
    using SafeERC20 for IERC20;
    using EnumerableSet for EnumerableSet.AddressSet;
    //definisce un evento SwapBack, che verrà emesso ogni volta che avviene uno scambio all'interno del contratto. L'evento contiene diversi parametri che rappresentano i risultati dello scambio.
    ...
    //definisce un evento Trade, che viene emesso ogni volta che avviene una transazione di trading all'interno del contratto. L'evento contiene informazioni sull'utente che ha effettuato il trade, il token scambiato, 
    //l'importo scambiato, il lato del trade, la quantità di token circolanti e un timestamp.
    event Trade(address user, address pair, uint256 amount, uint side, uint256 circulatingSupply, uint timestamp);
    //definisce un evento AddLiquidity, che viene emesso ogni volta che viene aggiunta liquidità al contratto.
    event AddLiquidity(uint256 tokenAmount, uint256 ethAmount, uint256 timestamp);

    bool public swapEnabled = true;
    bool public addLiquidityEnabled = true;

    bool public inSwap; //contralla se ci sono swap in corso
    modifier swapping() { //impedisce l'esecuzione simultanea di più scambi.
        inSwap = true;
        _;
        inSwap = false;
    }
    //Associa un booleano ad ogni indirizzo 
    //Viene utilizzato per determinare se un determinato indirizzo è esente da determinate commissioni o tasse all'interno del contratto
    //e se può aggiungere liquidità 
    mapping(address => bool) public isFeeExempt;
    mapping(address => bool) public canAddLiquidityBeforeLaunch;

    uint256 private burnFee;   // burn
    uint256 public gov1Fee;        //dividenti agli holders
    uint256 public gov2Fee;        //Flexible founds
    uint256 private liquidityFee; //refill AISIC-ETH liquidity pool
    uint256 private jackpotFee;   //jackpot
    uint256 private bonusFee;
    uint256 private devFee;      //developers fee
    uint256 private totalFee;
    uint256 public feeDenominator = 10000;

    // Buy Fees (percentuali)
    ...
    function _dogTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        if (inSwap) { //controlla se il contratto è impegnato in uno swap
        ...
        bool shouldTakeFee = (!isFeeExempt[sender] && !isFeeExempt[recipient]) && launched(); //check se deve pagare le tasse
        uint side = 0;
        address user_ = sender;
        address pair_ = recipient;
        // setta le fees in base al tipo di aquisto, se è acquisto o vendita
        if (isPair(sender)) {
            buyFees();
            side = 1;
            user_ = recipient;
    ...
    function swapBack() internal swapping {
        ...
        AISICBonusPool(bonusWallet).injectRewards(amountDogBonus);
    ...
    function clearStuckEthBalance() external onlyOwner {
        ...
        require(success, 'AISIC: ETH_TRANSFER_FAILED');
    }
